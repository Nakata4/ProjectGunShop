...
public class Welcome extends JPanel implements ChangeListener
{
	private GunSalesSystem gunSystem;
	private JLabel headingLabel = new JLabel("Welcome to the Gun Shop System", JLabel.CENTER);
	...
	public Welcome(GunSalesSystem gunSys, String data)
	{
		gunSystem = gunSys;
		file = data;
	...
	private void updateStats()
	{
		...
		double avgKm = Math.floor(gunSystem.getStatistics(GunSalesSystem.AVERAGE_DISTANCE) * 10 + 0.5) / 10;
		double avgAge = Math.floor(gunSystem.getStatistics(GunSalesSystem.AVERAGE_AGE) * 10 + 0.5) / 10;
		java.io.File f = new java.io.File(file);
		long size = f.length(); 

		gunsLabel.setText("Total number of guns: " + String.valueOf(guns));
		manufacturersLabel.setText("Total number of manufacturers: " + String.valueOf(manufacturers));
		avgPriceLabel.setText("Average gun price: " + String.valueOf(avgPrice));
		avgAgeLabel.setText("Average gun age: " + String.valueOf(avgAge));
